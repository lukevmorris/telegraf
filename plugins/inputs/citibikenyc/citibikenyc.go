...
func (n *CitiBikeNYC) SampleConfig() string {
	return sampleConfig
}

func (n *CitiBikeNYC) Description() string {
	return "Read CitiBikeNYC status and ridership information"
}

func (n *CitiBikeNYC) Gather(acc telegraf.Accumulator) error {
	if len(n.Urls) == 0 {
...
func (n *CitiBikeNYC) createHttpClient() (*http.Client, error) {
...
func (n *CitiBikeNYC) gatherUrl(addr *url.URL, acc telegraf.Accumulator) error {
	...
	acc.AddFields("citibikenyc", fields, tags)

	return nil
}

func (n *CitiBikeNYC) gatherScores(data string) map[string]interface{} {
...
// Get tag(s) for the citibikenyc plugin
...
func init() {
	inputs.Add("citibikenyc", func() telegraf.Input {
		return &CitiBikeNYC{}
	})
}
